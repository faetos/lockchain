...
*Setup instructions for a Gitian build of LockChain Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the LockChain
...
- [Building LockChain Core](#building-bitcoin-core)
...
------------------------------------------------
The instructions below use the automated script [gitian-build.py](https://github.com/lockchainchain/lockchain/blob/master/contrib/gitian-build.py) which only works in Debian/Ubuntu. For manual steps and instructions for fully offline signing, see [this guide](./gitian-building/gitian-building-manual.md).
...
```bash
cp lockchain/contrib/gitian-build.py .
```
...
```
git clone git@github.com:lockchainchain/gitian.sigs.git
...
export BUILD=0.14.9
gpg --output $VERSION-linux/$NAME/lockchain-linux-$BUILD-build.assert.sig --detach-sig $VERSION-linux/$NAME/bitcoin-linux-$BUILD-build.assert 
gpg --output $VERSION-osx-unsigned/$NAME/lockchain-osx-$BUILD-build.assert.sig --detach-sig 0$VERSION-osx-unsigned/$NAME/bitcoin-osx-$BUILD-build.assert 
gpg --output $VERSION-win-unsigned/$NAME/lockchain-win-$BUILD-build.assert.sig --detach-sig $VERSION-win-unsigned/$NAME/bitcoin-win-$BUILD-build.assert 
```

Make a PR (both the `.assert` and `.assert.sig` files) to the
[lockchainchain/gitian.sigs](https://github.com/lockchainchain/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lockchain-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/lockchain-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lockchain-osx-*-build.assert
```
