...
bool CMaxnodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 LCKC tx got MAXNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(maxvin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 LCKC tx -> 1 confirmation
