...
const QString BITCOIN_IPC_PREFIX("lockchain:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/lockchain-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/lockchain-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/lockchain-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        ...
        // If the lockchain: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // lockchain: URI
        {
                                                                       ...
                                                                       optionsModel(0)
{
    ...
    // on Mac: sent when you click lockchain: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start lockchain: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling lockchain: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject* object, QEvent* event)
{
    // clicking on lockchain: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in lockchain: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // lockchain: URI
    {
        ...
        } else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient)) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid LockChain address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
