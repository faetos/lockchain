...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a LockChain address (e.g. %1)").arg("S7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no LockChain: URI
...
{
    // Convert lockchain:// to lockchain:
    //
    //    Cannot handle this later, because lockchain:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open lockchain.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LockChain.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for LockChain.lnk
...
{
    return GetAutostartDir() / "lockchain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a lockchain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=LockChain\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lockchain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add lockchain app to startup item list
