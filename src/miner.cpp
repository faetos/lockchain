...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LockChainMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("LockChainMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrint("debug", "LockChainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lockchain-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        CBlockIndex* pindexPrev = chainActive.Tip();
        if (!pindexPrev) {
            LogPrint("debug", "LockChainMiner bailing, no pindexPrev\n");
            continue;
        }
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake));
        if (!pblocktemplate.get()) {
            LogPrint("debug", "LockChainMiner bailing, no pblocktemplate got\n");
            continue;
        } else {
            LogPrint("debug", "LockChainMiner proceeding with pblocktemplate.\n");
        }
        CBlock* pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrint("debug", "LockChainMiner skipping PoS section 4\n");
        //Stake miner main
        if (fProofOfStake) {
            LogPrint("debug", "LockChainMiner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());
            if (pblock->IsZerocoinStake()) {
                ...
                LogPrint("debug", "LockChainMiner(): Signing new block with UTXO key failed \n");
                continue;
            }

            LogPrint("debug", "LockChainMiner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
        ...
        LogPrint("debug", "Running LockChainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrint("debug", "Running LockChainMiner with hashTarget %0x\n", hashTarget.GetCompact());
        while (true) {
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrint("debug", "ThreadLockChainMiner() exception");
    }

    LogPrint("debug", "ThreadLockChainMiner exiting\n");
}
