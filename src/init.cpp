...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LYTX and zLYTX money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LYTX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LYTX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in LYTX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-stake=<n>", strprintf(_("Enable or disable staking functionality for LYTX inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zstake=<n>", strprintf(_("Enable or disable staking functionality for zLYTX inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupz=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLYTX minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zLYTX backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        uiInterface.InitMessage(_("Syncing zLYTX wallet..."));
