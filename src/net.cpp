...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "LockChain " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. LockChain Core is probably already running."), addrBind.ToString());
        else
...
void RelayInv(CInv& inv)
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes){
    		...
    		if((pnode->nServices==NODE_BLOOM_WITHOUT_MN) && inv.IsMaxNodeType())continue;
